...
class GenericDetailView(LoginRequiredMixin, UserPassesTestMixin, generic.DetailView):
    template_name = 'core/list_objects.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        obj.update_url = self.get_update_url(obj)
        obj.delete_url = self.get_delete_url(obj)
        obj.extra_url = self.get_extra_url(obj)
...
class MultipleSectionView(GenericDetailView):
    sections = []

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['sections'] = []
        for name, section in enumerate(self.sections):
            section_context = section.to_context()
            context['sections'].append(section_context)
        return context


class SecMultipleSectionView(MultipleSectionView):
...
class ScopedSecDetailView(GenericDetailView):
