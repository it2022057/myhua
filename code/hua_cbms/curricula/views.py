...
from accounts.checks import is_staff_member, is_secretariat
...
"""
Based on provided repository myfaculty_pub2 starter code.
Only the above autocomplete views were made for this implementation.
"""
...
#         obl[n + 1] = Course.objects.filter(program=program, semester=n + 1, active=True, elective=False).order_by(
#             'code_gr')
#         elec[n + 1] = Course.objects.filter(program=program, semester=n + 1, active=True, elective=True).order_by(
#             'code_gr')
#
#     semesters = [n + 1 for n in range(semester_max)]
#     return render(request, 'curricula/public_list_courses2.html',
#                   context={'obl': obl, 'elec': elec, 'semesters': semesters})
#
#
...
#
#     semesters = [n + 1 for n in range(semester_max)]
#     return render(request, 'curricula/public_list_courses2_en.html',
#                   context={'obl': obl, 'elec': elec, 'semesters': semesters})
#
#
